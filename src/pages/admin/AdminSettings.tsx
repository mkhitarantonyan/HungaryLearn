      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="space-y-2 text-xs text-gray-600"><div className="flex justify-between"><span>Пользователей Firestore</span><strong>{users.length}</strong></div><div className="flex justify-between"><span>Уроков в сборке</span><strong>{lessons.length}</strong></div></div>
          </div>
          <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="mb-3 text-sm font-bold">Сессия администратора</h2><p className="mb-4 text-xs leading-relaxed text-gray-500">Выход завершит Firebase-сессию на этом устройстве.</p><PrimaryButton onClick={handleLogout} className="w-full bg-red-600! hover:bg-red-700!"><LogOut className="w-4" />Выйти</PrimaryButton><div className="mt-3"><SecondaryButton onClick={() => navigate('/')} className="w-full">На главную</SecondaryButton></div></div>
        </div>
      </div>
    </div>
  );
